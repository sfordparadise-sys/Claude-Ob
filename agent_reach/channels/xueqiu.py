...
def _inject_cookie_string(cookie_str: str) -> None:
    for pair in cookie_str.split(";"):
        ...
        cookie = http.cookiejar.Cookie(version=0, name=name.strip(), value=value.strip(), port=None, port_specified=False, domain=".xueqiu.com", domain_specified=True, domain_initial_dot=True, path="/", path_specified=True, secure=True, expires=None, discard=True, comment=None, comment_url=None, rest={})
...
class XueqiuChannel(Channel):
    ...
    def check(self, config=None):
        ...
        try:
            ...
            return "warn", f"Xueqiu API 连接失败：{e}。请先登录雪球后运行：agent-reach configure --from-browser chrome"
    ...
    def search_stock(self, query: str, limit: int = 10) -> list:
        ...
        return [{"symbol": s.get("code", ""), "name": s.get("name", ""), "exchange": s.get("exchange", "")} for s in stocks[:limit]]

    def get_hot_posts(self, limit: int = 20) -> list:
        data = _get_json("https://xueqiu.com/v4/statuses/public_timeline_by_category.json?since_id=-1&max_id=-1&count=20&category=-1")
        ...
        for item in items[:limit]:
            ...
            results.append({"id": post.get("id", 0), "title": post.get("title") or "", "text": text[:200], "author": user.get("screen_name", ""), "likes": post.get("like_count", 0), "url": f"https://xueqiu.com{target}" if target else ""})
        return results
    ...
    def get_hot_stocks(self, limit: int = 10, stock_type: int = 10) -> list:
        ...
        items = (data.get("data") or {}).get("items") or []
        return [{"symbol": item.get("code") or item.get("symbol", ""), "name": item.get("name", ""), "current": item.get("current"), "percent": item.get("percent"), "rank": idx} for idx, item in enumerate(items[:limit], 1)]
